...
- [x] **GitHub Actions Secrets の確認**(10–20分): 値の一覧は `docs/project-management.md` の「Verify GitHub Actions production secrets」参照
- [ ] **実メールでの Cognito サインアップ・スモークテスト**(15–30分)【保留・2026-07-12: `components/AuthPanel.tsx` にサインアップUIが存在せず、サインイン/状態確認/サインアウトのみ。自己登録するにはAWSコンソールまたは`aws cognito-idp admin-create-user`でテストユーザーを先に作る必要あり。方法を決めてから再開】: 登録メール到着 → 確認 → サインイン → 記録1件 → リロードで壊れない。**T45(認証 SDK の遅延読み込み)後の確認も兼ねる**: サインイン成功後に記録すると「バックアップが完了しました。」が表示されること・サインアウト後は同期文言が出ないこと
