...
- [x] **実メールでの Cognito サインアップ・スモークテスト**(15–30分): 2026-07-12 完了。本番 `veai.jp/carequest/` で `irevail8@gmail.com` を使い実機検証:新規登録(道B/T47)→ 確認コード到着(**メール到達性も実証**)→ 確認「登録が完了しました」→ サインインで「記録をクラウドにバックアップしました。」(T10 Phase B 自動同期が発火)→ 「ログイン済み: irevail8@gmail.com」。T45/T10/T47 が本番で end-to-end 動作することを確認。残: オフライン SW 表示の実機確認(下記)のみ
