...
## 第4バッチ(2026-07-10 立案)【テーマ: 堅牢性】— T17〜T20 完了、T21〜T22 は下記参照

完了: T17 UTC日付バグ修正 `64471e0` / T18 Vitest+CI(52→59テスト) `075f040` / T19 ストレージ検証・移行 `81e9bcf` / T20 エラーバウンダリ `8e854b6`

### 【Opus/Sonnet 単独セッション向け】残り2タスクの実行方法

Fable を介さず、Sonnet または Opus をメインモデルにした Claude Code セッションで次のように指示すれば実行できます(このリポジトリの `.claude/agents/carequest-worker.md` と `.claude/skills/` が自動で規律を与えます):

> docs/task-list.md の T21(または T22)を、受け入れ条件どおりに実装してください。まず AGENTS.md と .claude/skills/carequest-dev/SKILL.md、carequest-product-tone/SKILL.md を読んでから着手。コミットメッセージは既存スタイルに合わせ、1タスク1コミットで。完了後は npm test / lint / build の結果を報告してください。

### T22. 記録の JSON インポート(復元)機能(US-503 後半)| P1 | おすすめ度 ★★★ | 推奨モデル: Opus

- 背景: T13 のエクスポートと対で「端末を替えても記録を持ち込める」が完結し、R-02(localStorage 消失)の実質的な軽減策になる
- [ ] ふりかえり画面の「自分の記録を手元に残す」セクションに「記録を読み込む」を追加(`<input type="file" accept=".json">`)
- [ ] 取り込んだ JSON は **lib/storage.ts の sanitize/migration パイプラインを必ず通す**(T19 で導入済み。生 JSON を直接 saveCareState に渡さない)
- [ ] `app.name === "Care Quest"` の検証。不一致・破損時は穏やかなメッセージ(赤エラーにしない)
- [ ] マージ方針: 既存記録とインポート記録は **id で重複排除して統合**(上書き全消しにしない)。note は「既存が空のときだけ取り込む」等の安全側。方針はコードコメントに明記
- [ ] 取り込み結果を穏やかに表示(「◯件の記録を読み込みました」)
- [ ] インポートのユニットテストを lib/__tests__/ に追加(重複排除・破損 JSON・他アプリ JSON 拒否)
- [ ] npm test / lint / build green
- ★★★の理由: データ防御(第4バッチのテーマ)の最後のピース。Opus 推奨の理由: マージ・重複排除の設計ミスがユーザーデータ破壊に直結するため

### T21. E2E スモークテスト(Playwright)| P2 | おすすめ度 ★★ | 推奨モデル: Sonnet
