...
## 第4バッチ(2026-07-10 立案)— 推奨・未着手【テーマ: 堅牢性(バグの出ない固い仕組み)】

オーナー方針「なるべくアプリに穴がないように、バグの出ない固い仕組みに」を受けたバッチ。立案時のコード点検で**実バグを1件発見**(T17)。PM レンズ: PMP 品質(テスト・欠陥予防)+ ITIL 問題管理(根本原因の仕組み化)。リスク登録簿に R-10(回帰リスク)を追加。

### T17. 【バグ修正】日付が UTC 基準で深夜の記録が前日扱いになる | P0 | おすすめ度 ★★★

- 発見事項: `lib/storage.ts` の `getTodayDate()` が `toISOString()`(UTC)を使っており、**JST の 00:00〜08:59 の記録が前日の日付になる**。`lib/stats.ts` の履歴集計も同様。深夜介護(夜間対応)の記録が多い想定ユーザーに直撃する
- [ ] `getTodayDate()` をローカルタイムゾーン基準に修正し、日付ユーティリティを1か所に集約
- [ ] `getRecentDaySummaries` の日付計算も整合させる
- [ ] 過去データの扱いを明記(保存済みの date 文字列は移行しない。境界シフトで1日分の集計が一度ずれる可能性をレポートに記載)
- 委任: worker(**sonnet**、受け入れ条件を厳密に)/ ★★★の理由: 実ユーザー影響のある確定バグ。堅牢化はまず既知の穴を塞ぐことから
- 見積り: 2h

### T18. ユニットテスト基盤(Vitest)+ CI 組み込み | P1 | おすすめ度 ★★★

- [ ] Vitest 導入(devDependency)。`lib/` の純関数にテスト: messages(まとめ生成・励まし)、stats(日別集計・境界日付)、storage(後方互換ロード)、export(スキーマ)
- [ ] T17 の日付境界(深夜・月替わり)の回帰テストを必ず含める
- [ ] `npm test` を package.json に追加し、`.github/workflows/ci.yml` に組み込む
- 委任: worker(**sonnet**)/ ★★★の理由: 現在テストが0本。今後ワーカー並列開発を続ける以上、回帰の自動検知は体制の前提条件(R-10)
- 見積り: 3h

### T19. localStorage スキーマ検証とバージョン付きマイグレーション | P1 | おすすめ度 ★★★

- [ ] ロード時に各フィールドの型を検証し、壊れたデータは安全なデフォルトに置換(全捨てしない。救える記録は救う)
- [ ] スキーマバージョン番号を state に導入し、将来の構造変更をマイグレーション関数で吸収する仕組み
- [ ] 破損時もユーザーには穏やかな表示(データ喪失を黙って起こさない)
- 委任: worker(**opus**)/ ★★★の理由: localStorage が唯一の保存先である以上、データ整合性がこのアプリ最大の急所(R-02 直結)。設計の質が問われるため Opus
- 見積り: 3〜4h

### T20. エラーバウンダリとやさしいフォールバック | P2 | おすすめ度 ★★

- [ ] `app/error.tsx` / `app/global-error.tsx` / `app/not-found.tsx` を追加(Next.js 16 の規約を docs で確認)
- [ ] 例外時も白画面にせず、トーンガイド準拠の穏やかな画面+ホームへの導線(記録データは localStorage にあるため「消えていない」ことを伝える)
- 委任: worker(**sonnet**)/ ★★の理由: 発生確率は低いが、疲れた介護者に白画面は最悪の体験。コストが小さく保険価値が高い
- 見積り: 2h

### T21. E2E スモークテスト(Playwright)| P2 | おすすめ度 ★★

- [ ] 主要3フロー: ①記録→リロード→残っている ②休息モードのオン/オフ ③エクスポートのダウンロード
- [ ] ローカル(next dev)で動作し、CI にジョブ追加(まず PR 時のみで可)
- 委任: worker(**sonnet**)/ ★★の理由: ユニットテストでは拾えない画面間の回帰を検知。ただし T18 より導入・維持コストが高いため後回しでも可
- 見積り: 3〜4h

実行順の推奨: T17(確定バグ)→ T18(回帰防止の網)→ T19 → T20 → T21。「実行お願いします」でいつもの体制で進めます。
