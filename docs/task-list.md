...
- 背景: 一次ターゲットは40〜60代、高齢の介護者も想定(内部戦略ノート)。基礎的な a11y はこの層への品質そのもの
...
内部戦略ノート(非公開)のユーザーストーリーから「本番インフラに触れず、今すぐ完了・検証できるもの」を5つ選んで実行するリストです。GitHub Issues に転記する場合は `docs/project-management.md` のフィールド(Priority / Area / Owner / Estimate)を使ってください。
